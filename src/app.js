...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Sendet das Verifizierungs-Panel in diesem Channel'),

  async execute(interaction) {
    const channel = interaction.channel;
    if (!channel || !channel.isTextBased()) {
      return interaction.reply({ content: 'Dieser Channel ist kein Text-Channel.', ephemeral: true });
    }

    const panel = config.verifyPanel;
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(panel.descriptionLines.join('\n'))
      )
      .addSeparatorComponents(new SeparatorBuilder())
      .addMediaGalleryComponents(
        new MediaGalleryBuilder().addItems([{ media: { url: panel.imageUrl } }])
      )
      .addSeparatorComponents(new SeparatorBuilder());
    ...
    await interaction.reply({
      content: `${config.emojis.success} Verifizierungs-Panel wurde in diesem Channel gesendet.`,
      ephemeral: true,
    });
  },
};
